class FeedsController < ApplicationController
  ...
  def more
    ...
    publications = User.asker(params[:id]).publications.where("CREATED_AT < ? AND ID != ?", post.created_at, post.id).order("created_at DESC").limit(5).includes(:question => :answers)
