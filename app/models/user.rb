class User < ActiveRecord::Base
  ...
  has_many :follows_with_inactive, :through => :relationships, :source => :followed
  ...
  def get_my_questions_answered_this_week_count
    ...
    after_new_user_filter if transition.segment_type == 1 and transition.from_segment.blank? and transition.to_segment.present?
		...
		Post.trigger_split_test(id, "replace related feeds with top contributors (lifecycle+)") if transition.segment_type == 1 and transition.is_positive? and transition.is_above?(1)
	end
