...
describe AskersController, '#index' do
  ...
  let(:published_asker) { create :asker, published: true }
  let(:unpublished_asker) { create :asker, published: false }
  ...
  it "defaults to returning un/published askers with no published param" do
    published_asker
    unpublished_asker

    get :index, format: :json

    askers = JSON.parse(response.body)
    askers.count.must_equal 3
  end

  it "returns only published askers if passed param" do
    published_asker
    unpublished_asker
    ...
    askers = JSON.parse(response.body)
    ids = askers.map{|a| a['id']}

    askers.count.must_equal 1
    ids.must_include published_asker.id
    ids.wont_include unpublished_asker.id
  end

  it "returns only unpublished askers if passed param" do
    published_asker
    unpublished_asker
    ...
    askers.count.must_equal 1
    ids = askers.map{|a| a['id']}
    ids.must_include unpublished_asker.id
    ids.wont_include published_asker.id
  end
end
